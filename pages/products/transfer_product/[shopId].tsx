...
const TransferProduct: NextPage = () => {
  ...
  useEffect(() => {
    let data: any = localStorage.getItem("bactToolData");
  ...
  const onHandlerChangeForm = (e: any, data: any) => {
    globalE = e;
    let checking: any = qtyData.find((val) => val.shop_product_id == data.id);
    if (checking !== undefined) {
      setUpdateQtyData({ ...checking, quantity: e });
    } else {
      let val = qtyData.concat({
        shop_id: "",
        quantity: e,
  ...
  useEffect(() => {
    let isEmpty = Object.keys(updateQtyData).length !== 0;
    if (isEmpty) {
      let data = [];
      let val = qtyData.concat(updateQtyData);
      val.forEach((x) => {
        let find;
        if (
  ...
  return (
    <div>
      <VendorDashBoardLayout>
        <div
          style={{ display: "flex", marginBottom: "30px", marginTop: "-40px" }}
        >
        ...
        >
          ...
          {(
            <div>
              <div style={{ marginTop: "-30px", paddingLeft: "30%" }}>
                <svg
                  width="50"
                  height="65"
                  viewBox="0 0 71 75"
